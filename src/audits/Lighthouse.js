...
class Lighthouse {
 ...
 async start() {
  this.spinner = ora( `Start Lightouse audit for ${this.url}` ).start();
  this.spinner = this.spinner.stop()
  const results = await this.runPSI();
  const output = this.formatOutput( results );
  this.spinner.succeed( `Finish Lightouse audit for ${this.url}` )
  return output
 }
 ...
 async runPSI() {
  ...
  for ( var strategy of this.config.strategies ) {
   const singleSpinner = ora( `Start ${strategy} Lighthouse audit` ).start();
   const singleResult = await this.runSinglePSI( Object.assign( {
    strategy,
   }, this.config.options ) )
   results.push( singleResult )
   singleSpinner.succeed( `Finish ${strategy} Lighthouse audit` )
  }
  return results
 }
