...
class Pagespeed {
  ...
  renderResult(data) {
    ...
    const resultsAudits = Object.values(audits).map((element) => {
      ...
      if (score === 1) return "";
      let _results = `### ${title} - Score: ${
        score === null ? "not-scored" : score
      }  \n`;
      _results += `> ${description} \n`;
      return _results;
    });
    const resultPeformance = Object.keys(metrics).map((key) => {
      return `> Type: ${key} - Percentile: ${metrics[key].percentile} - Result: ${metrics[key].category} \n`;
    });
